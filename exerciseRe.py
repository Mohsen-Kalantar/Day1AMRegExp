...
def parseDate(inputText):
    """
    parseDate returns a date given as an argument a string
    representing the date
        argument: inputText a str representing the date
        returned value: a datetime.date
        exception ParseDateError can be raised if the argument is not
        a valid date.
    """
...
def parseTime(inputText):
    reg=re.compile(r"^([0-2]\d)[H:]([0-5]\d)([M:]([0-5]\d))?$", re.IGNORECASE|re.VERBOSE)
    mo=reg.search(inputText)
    if mo:
